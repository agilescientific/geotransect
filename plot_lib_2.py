...
def uberPlot(transect, seismic_container, elevation_container,
    ...
    description = fig.add_subplot(gs[1:4, 0:1 * mw / 4])
    large_scale = fig.add_subplot(gs[0:5, 3:16])
    small_scale = fig.add_subplot(gs[2:4, 14:16])
    feat_header = fig.add_subplot(gs[0:1, -5:])
    ns_label = fig.add_subplot(gs[-1:, -5:])
    ns_logo = fig.add_subplot(gs[-1:, -2:-1])
    ...
    for coords, data in zip(seismic_x, seismic_data):
        ...
        xsection.imshow(data,
                        extent=[np.amin(coords) / 1000.0,
                                ...
                                depth, z0],
                        aspect="auto", cmap="Greys", alpha=1.0)
    ...
    for las, pos in zip(log_data, log_x):
        ...
        data *= lgsc * (extents[1] / 1000. - extents[0] / 1000.)
        data += pos

        log.plot(data, las.data['DEPT'],
                 'g', lw=0.5, alpha=0.5, zorder=10)

        log.set_xlim((extents[0] / 1000., extents[1] / 1000.))
        log.set_ylim((extents[2], extents[3]))
        log.patch.set_facecolor('w')
        log.patch.set_alpha(0.0)
    ...
    for i, geo in enumerate(bedrock_data[:-1]):
        ...
        elevation.plot(elevation_x[idx], elevation_data[idx],
                       linewidth=5, color=color.flatten())
    ...
    imglrg = Image.open("data/NS_large.png")
    arrlrg = np.array(imglrg)
    large_scale.imshow(arrlrg,
                        # aspect="equal",
                        interpolation='none')
    ...
    """
    # large_scale.plot(imlrg, aspect="equal")
    ...
    """
    imgSm = Image.open("data/NS_Small.png")
    arrSm = np.array(imgSm)
    large_scale.imshow(arrSm, aspect="equal", interpolation='none')
    large_scale.patch.set_edgecolor("0.90")
    large_scale.axis('off')
    """
    ...
    em.plot(em_x, em_data, 'b', alpha=0.5, lw=2)
    em.axis("off")
    ...
    # --------------------------------------------------------------#
    # LOGO and Label in bottom right
    # --------------------------------------------------------------#
    ...
    # Broken
    ns_logo.imshow(arr, aspect="equal", zorder=1)
    ns_logo.axis('off')
